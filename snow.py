...
p_mass = p_vol * p_rho
E, nu = 4.8e4, 0.2 # Young's modulus and Poisson's ratio
mu_0, lambda_0 = E / (2 * (1 + nu)), E * nu / ((1+nu) * (1 - 2 * nu)) # Lame parameters
xi = 10 # Hardness parameter
...
F = ti.Matrix.field(3, 3, dtype=float, shape=n_particles) # elastic deformation gradient
...
def kirchoff_FCR(F, R, J, mu, la):
  return 2 * mu * (F - R) @ F.transpose() + ti.Matrix.identity(float, 3) * la * J * (J - 1) #compute kirchoff stress for FCR model
...
def substep():
  ...
  for p in x: 
    ...
    mu, la = mu_0 * ti.exp(xi * (1 - J_pl)), lambda_0 * ti.exp(xi * (1 - J_pl))
  ...
  for p in x: 
    ...
    F_trial = (ti.Matrix.identity(float, 3) + (dt * new_F)) @ F[p] #updateF (explicitMPM way)
    
    U, sig, V = ti.svd(F_trial)
    sig_c = ti.Matrix.identity(float, 3)
    for d in ti.static(range(3)):
      new_sig = clamp(sig[d, d])
      sig_c[d, d] = new_sig
    F[p] = U @ sig_c @ (V.transpose())
    F_p[p] = ((F[p]).inverse()) @ F_trial @ F_p[p]
